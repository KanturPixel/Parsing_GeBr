...
class Book24Spider(scrapy.Spider):
    ...
    def parse(self, response=HtmlResponse):
        ...
        links = response.xpath('//div[contains(@class, "product-card_content")]/a/@href').getall()
        for link in links:
            yield response.follow(parent_url + link, callback=self.parse)

        next_page = response.xpath('//li[contains(@class, "pagination_button-item")]//a//@href').get()
        if next_page:
